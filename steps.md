...
&nbsp;       \* \[ ] Salvar o `refresh\_token` criptografado e o `access\_token` no banco de dados, associados ao `deviceId`.
...
&nbsp;   \* \[ ] Criar uma rota protegida `POST /drive/upload`.
...
&nbsp;   \* \[ ] Instalar dependências (`axios`, `@capacitor-community/sqlite`).
...
&nbsp;       \* \[ ] Criar um componente de UI (botão "Login com Google").
